...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # we have self.startingPosition and self.corners 
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        visitedSet = set(state[1])
        return len(self.cornersSet.difference(visitedSet)) == 0
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx, nexty = int(x + dx), int(y + dy)
            if not self.walls[nextx][nexty]:
                if ((nextx, nexty)) in self.cornersSet:
                    visited.add((nextx, nexty))
                visited = tuple(visited)    
                nextState = ((nextx, nexty), visited)
                cost = 1 # no cost function
                successors.append( ( nextState, action, cost) )

        # note: costFn is unavailable ==> cost = 1 uniformly
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # first idea: manhattan distance to the furthest corners
    cornersSet = set(corners)
    visitedSet = set(state[1])
    remainCorners = cornersSet.difference(visitedSet)
    distances = []
    position = state[0]
    for eachCorner in remainCorners:
        xy1 = position
        xy2 = eachCorner
...
# test Min Max Manhattan
#points = [(1, 6), (2, 2), (6, 5), (5, 1)]
#points = []
#position = (4, 3)
#
#minManhattan, minPoint = findMinManhattan(position, points)
#maxManhattan, maxPoint = findMaxManhattan(position, points)
#print minManhattan, minPoint
#print maxManhattan, maxPoint
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x,y = state
        # don't know if I need to update the food grid
